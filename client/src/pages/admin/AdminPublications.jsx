...
import { publicationsAdmin } from "../../api/client";
import {
  Card,
  Field,
  Input,
...
export default function AdminPublications() {
  ...
  async function load() {
    setLoading(true);
    setItems(await publicationsAdmin.list());
    setLoading(false);
  }
  useEffect(() => {
    load();
  ...
  async function handleSave() {
    ...
    if (!editing.fileUrl) {
      setError("Please upload the publication file before saving");
      return;
    }
    setSaving(true);
    setError("");
    try {
      if (isNew) await publicationsAdmin.create(editing);
      else await publicationsAdmin.update(editing._id, editing);
      setEditing(null);
      await load();
    } catch (err) {
  ...
  async function handleDelete(id) {
    ...
    await publicationsAdmin.remove(id);
    await load();
  }
      ...
      <div className="max-w-2xl space-y-6">
        ...
        <Card>
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <Field label="Authors">
                <Input
                  ...
                  placeholder="e.g. 'R. Sharma, A. Rai'"
                />
              </Field>
            ...
            <Field
              label="File"
              hint="Upload the paper as a PDF or Word document"
            >
              <FileUpload
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-navy-500 mt-1">
            Faculty and institutional research publications, shown on the
      ...
      {loading ? (
        ...
        <Card>
          <EmptyState text="No publications yet — add the first one." />
        </Card>
      ) : (
